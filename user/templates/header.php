...
if (!empty($foto_db)) {
    ...
    $foto_profil = '../assets/img/avatar-santri.png'; // Default Avatar
}
...
<head>
    ...
    <style>
        ...
        /* --- LOGIKA COLLAPSED SIDEBAR --- */
        #sidebar.collapsed {
            width: 5rem;
        }

        /* Sembunyikan Teks Menu & Group Title saat Collapsed */
        ...
        /* --- LOGIKA LOGO SWITCHING --- */
        /* Default: Logo Full Muncul, Logo Icon Sembunyi */
        .logo-full {
            display: block;
            opacity: 1;
        ...
        /* Saat Collapsed: Logo Full Sembunyi, Logo Icon Muncul */
...
<body class="bg-slate-50 text-slate-800 antialiased overflow-hidden">
    ...
    <div class="flex h-screen relative">
        ...
        <aside id="sidebar"
            class="sidebar-transition absolute lg:relative z-50 w-64 h-full bg-white border-r border-slate-200 flex flex-col transform -translate-x-full lg:translate-x-0">

            <div class="h-20 flex items-center px-6 border-b border-slate-100 bg-primary-900 text-white relative">
                <div class="w-full flex items-center h-full overflow-hidden">
                    <div class="logo-full">
                        <img src="../assets/img/images/admin-ajax_2.png" alt="Logo Full"
                            class="h-10 w-auto object-contain">
                    </div>

                    <div class="logo-icon">
                        <img src="../assets/img/images/logo-pondok.png" alt="Logo Icon"
                            class="h-10 w-auto object-contain">
                    </div>
                </div>
            ...
            <div class="p-4 border-t border-slate-100 bg-slate-50">
                <a href="../logout.php"
                    class="flex items-center px-4 py-3 rounded-xl text-red-600 hover:bg-red-50 hover:text-red-700 transition-colors w-full group menu-link">
                    <i
                        class="fas fa-sign-out-alt w-6 text-center group-hover:rotate-180 transition-transform duration-300"></i>
            ...
            <header class="h-20 glass-header flex items-center justify-between px-6 sticky top-0 z-30 shadow-sm">
                <div class="flex items-center gap-4">
                    <button id="sidebarToggleBtn"
                        class="p-2 rounded-lg text-slate-500 hover:bg-slate-100 hover:text-primary-700 transition-colors focus:outline-none">
                        <i class="fas fa-bars text-xl"></i>
                    </button>
                    <div>
                        <h2 class="text-xl font-bold text-slate-800 tracking-tight"><?= htmlspecialchars($page_title) ?>
                        </h2>
                    ...
                    <div class="relative group">
                        <button class="flex items-center gap-3 focus:outline-none">
                            <div class="text-right hidden sm:block">
                                ...
                                <span
                                    class="text-[10px] font-semibold text-primary-600 bg-primary-100 px-2 py-0.5 rounded-full inline-block mt-1">Santri</span>
                            </div>
                            <div class="relative">
                                <img src="<?= htmlspecialchars($foto_profil) ?>" alt="Profile"
                                    class="w-10 h-10 rounded-full object-cover border-2 border-white shadow-md group-hover:border-primary-400 transition-colors">
                                <div
                                    class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white rounded-full">
                                </div>
                            </div>
                            <i
                                class="fas fa-chevron-down text-slate-400 text-xs sm:block hidden group-hover:text-primary-600 transition-colors"></i>
                        </button>
                        <div
                            class="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-slate-100 py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-right z-50">
                            <div class="px-4 py-3 border-b border-slate-100 bg-slate-50/50">
                                <p class="text-xs text-slate-500">Login sebagai</p>
                                <p class="text-sm font-bold text-slate-800 truncate"><?= htmlspecialchars($nama_user) ?>
                                </p>
                            </div>
                            <a href="profil.php"
                                class="block px-4 py-2 text-sm text-slate-700 hover:bg-primary-50 hover:text-primary-700"><i
                                    class="far fa-user mr-2 w-4"></i> Edit Profil</a>
                            <div class="border-t border-slate-100 my-1"></div>
                            <a href="../logout.php"
                                class="block px-4 py-2 text-sm text-red-600 hover:bg-red-50 hover:text-red-700 font-medium"><i
                                    class="fas fa-sign-out-alt mr-2 w-4"></i> Keluar</a>
                        </div>
                    </div>
                </div>
            </header>
