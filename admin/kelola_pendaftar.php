...
<div id="modalPilihPeserta" class="fixed inset-0 z-[60] hidden bg-emerald-900/40 backdrop-blur-sm flex items-center justify-center p-4 transition-all duration-300">
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[85vh] transform scale-95 opacity-0 transition-all duration-300" id="modalPesertaContent">
        <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-emerald-900 text-white relative overflow-hidden">
            ...
            <button onclick="closeModalPeserta()" class="relative z-10 text-white/70 hover:text-white hover:bg-white/20 rounded-full w-8 h-8 flex items-center justify-center transition-colors">
                <i class="fas fa-times text-lg"></i>
            </button>
        </div>

        <div class="flex-1 overflow-hidden flex flex-col bg-white">
            <form action="proses_tambah_peserta_internal.php" method="POST" id="formInternal" class="flex flex-col h-full">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead class="bg-gray-50 text-gray-500 text-xs font-bold uppercase tracking-wider sticky top-0 z-10 shadow-sm">
                            <tr>
                                ...
                                <th class="p-4 border-b border-gray-100">Info</th>
...
<script>
    ...
    function openModalPeserta(workshopId, judulEvent) {
        ...
        // Panggil AJAX ke endpoint pencari santri
    ...
    window.toggleRow = function(row) {
        ...
        // Prevent toggle if clicking directly on checkbox to avoid double toggle
