...
// 1. PROSES TOGGLE STATUS DISKUSI
...
<div class="min-h-screen bg-gray-50 font-sans pb-20">
    ...
    <div class="bg-emerald-900 pb-20 pt-10 px-4 rounded-b-[3rem] shadow-xl relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-6 relative z-10">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <span class="text-emerald-200 text-xs font-bold uppercase tracking-widest border border-emerald-700/50 px-2 py-1 rounded-md">Moderasi</span>
                    <h1 class="text-3xl md:text-4xl font-extrabold text-white mt-2 leading-tight">Ruang Diskusi</h1>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 -mt-16 relative z-20 space-y-8">
        ...
        <?php if (count($workshops) > 0): ?>
            ...
            <div class="flex overflow-x-auto gap-3 pb-4 scrollbar-hide snap-x">
                <?php foreach ($workshops as $ws): ?>
                    ...
                    <a href="?active_ws=<?= $ws['workshop_id'] ?>" class="snap-start shrink-0 px-5 py-3 rounded-2xl text-sm font-bold transition-all border flex items-center gap-3 shadow-md 
                       <?= $isActive ? 'bg-white text-emerald-800 border-emerald-500 ring-2 ring-emerald-500/20' : 'bg-white/90 backdrop-blur-sm text-gray-500 border-transparent hover:bg-white' ?>">
                        <?php if ($ws['poster']): ?>
                            <img src="../assets/img/posters/<?= htmlspecialchars($ws['poster']) ?>" class="w-8 h-8 rounded-lg object-cover bg-gray-200">
                        <?php else: ?>
                            <div class="w-8 h-8 rounded-lg bg-emerald-100 flex items-center justify-center text-emerald-600"><i class="fas fa-chalkboard-teacher"></i></div>
                        <?php endif; ?>
                        <div class="flex flex-col truncate max-w-[150px]">
                            ...
                            <?php if (!$ws['is_diskusi_active']): ?>
                                <span class="text-[10px] text-red-500 flex items-center gap-1 font-normal"><i class="fas fa-lock"></i> Terkunci</span>
                            <?php endif; ?>
                        </div>
                    </a>
                <?php endforeach; ?>
            ...
            if ($current_ws):
                // QUERY CHAT: Join ke 2 tabel (users & santri) berdasarkan user_type
                $stmt_d = $pdo->prepare("
                    SELECT d.*, 
                ...
                <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden flex flex-col h-[700px] relative">

                    <div class="bg-white/80 backdrop-blur-md px-6 py-4 border-b border-gray-100 flex justify-between items-center sticky top-0 z-10">
                        <div>
                            <h3 class="font-bold text-gray-800 text-lg flex items-center gap-2">
                                <?= htmlspecialchars($current_ws['judul']) ?>
                                <?php if ($current_ws['is_diskusi_active']): ?>
                                    <span class="text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full border border-emerald-200">Aktif</span>
                                <?php else: ?>
                                    <span class="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded-full border border-red-200"><i class="fas fa-lock mr-1"></i> Terkunci</span>
                                <?php endif; ?>
                            </h3>
                            <p class="text-xs text-gray-500 mt-0.5 flex items-center gap-2">
                                <i class="fas fa-comments text-amber-500"></i> <span id="chatCount"><?= count($chats) ?></span> Pesan
                            </p>
                        </div>
                        ...
                        <form method="POST">
                            ...
                            <button type="submit" name="toggle_diskusi" class="px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-sm flex items-center gap-2 
                                <?= $current_ws['is_diskusi_active'] ? 'bg-red-50 text-red-600 hover:bg-red-100 border border-red-200' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100 border border-emerald-200' ?>">
                                <?php if ($current_ws['is_diskusi_active']): ?>
                    ...
                    <div class="flex-grow overflow-y-auto p-6 space-y-4 bg-slate-50 scrollbar-thin scrollbar-thumb-gray-200" id="chatContainer">
                        <?php if (count($chats) > 0): ?>
                            <?php foreach ($chats as $chat):
                                // Logic menentukan siapa pengirim
                                $is_me = ($chat['user_type'] == 'admin' && $chat['user_id'] == $user_id);
                                $is_admin_lain = ($chat['user_type'] == 'admin' && !$is_me);
                                
                                if ($is_me) {
                                    // Saya (Admin) -> Kanan, Hijau
                                    ...
                                    $name = 'Anda (Admin)';
                                } elseif ($is_admin_lain) {
                                    // Admin Lain -> Kiri, Kuning
                                    ...
                                    $name = htmlspecialchars($chat['nama_lengkap']) . ' <i class="fas fa-crown text-amber-500 ml-1" title="Admin"></i>';
                                } else {
                                    // Santri -> Kiri, Putih
                                    $wrapperClass = 'justify-start';
                                ...
                                <div class="flex <?= $wrapperClass ?> group animate-fade-in">
                                    <div class="max-w-[85%] sm:max-w-[75%]">
                                        <div class="flex items-center gap-2 mb-1 px-1 <?= $is_me ? 'flex-row-reverse' : '' ?>">
                                            <span class="text-xs font-bold <?= $metaClass ?>"><?= $name ?></span>
                                            <span class="text-[10px] text-gray-400"><?= date('H:i', strtotime($chat['created_at'])) ?></span>
                                        </div>
                        ...
                        <?php if (!$current_ws['is_diskusi_active']): ?>
                            <div class="bg-red-50 px-4 py-2 flex justify-between items-center border-b border-red-100">
                                <div class="text-xs font-bold text-red-600 flex items-center gap-2">
                                    <i class="fas fa-lock"></i> Diskusi dikunci untuk peserta.
                                </div>
                                <form method="POST" class="inline">
                                    <input type="hidden" name="workshop_id" value="<?= $active_ws_id ?>">
                                    <input type="hidden" name="current_status" value="0">
                                    <button type="submit" name="toggle_diskusi" class="text-xs text-red-700 hover:text-red-900 underline font-bold">Buka Diskusi</button>
                                </form>
                            </div>
                        <?php endif; ?>
                            ...
                            <form id="chatForm" class="flex gap-3 relative items-end">
                                ...
                                <div class="relative flex-grow">
                                    <textarea name="pesan" required rows="1" id="chatInput"
                                        ...
                                        placeholder="Ketik pesan sebagai Admin..."></textarea>
                                </div>
        ...
        <?php else: ?>
            <div class="flex flex-col items-center justify-center py-20 bg-white rounded-3xl shadow-lg border border-gray-100 text-center">
                ...
                <a href="form_event.php" class="bg-emerald-600 text-white px-6 py-2.5 rounded-xl font-bold hover:bg-emerald-700 transition">Buat Event Baru</a>
            </div>
        <?php endif; ?>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (chatForm) {
            chatForm.addEventListener('submit', async function (e) {
                ...
                try {
                    const response = await fetch('ajax_chat_send.php', { method: 'POST', body: formData });
                    const result = await response.json();

                    if (result.status === 'success') {
                        document.getElementById(`time-${tempId}`).innerText = result.timestamp;
                    } else {
        ...
        function createBubble(text, id, isPending) {
            ...
            <div class="max-w-[85%] sm:max-w-[75%]">
                <div class="flex items-center gap-2 mb-1 px-1 flex-row-reverse">
                    ...
                    <span class="text-[10px] text-gray-400" id="time-${id}">${timeDisplay}</span>
                </div>
        ...
        function markBubbleFailed(id) {
            ...
            if (bubble) {
                const inner = bubble.querySelector('.bg-emerald-600');
                inner.classList.replace('bg-emerald-600', 'bg-red-500');
                inner.title = "Gagal terkirim";
                document.getElementById(`time-${id}`).innerHTML = '<i class="fas fa-exclamation-circle text-red-500"></i>';
            }
        }
    });
</script>
